...
export class AllData extends React.Component {
    ...
    editUpdateDetail(allData, id) {
        if (document.getElementById(id).textContent === "Edit Details") {
            ...
            console.log(" edit detail", allData.id);
            var url = 'http://localhost:1212/api/' + allData.id + '';
            fetch(url, {
                method: 'GET',
                headers: {
                    Accept: 'application/json',
                ...
                .then((editData) => {
                    ...
                    document.getElementById(editData.name).readOnly = false;
            ...
            var name = document.getElementById(allData.fname).value;
    ...
    renderProducts() {
        return this.state.allData.map((allData, i) => {
            return (
                <tr key={i} id={allData.id}>
                    {/*<td>{allData._id}</td>*/}
                    <td><input type="text" id={allData.name} defaultValue={allData.name} readOnly={true} /></td>
                    ...
                    <td><button to="/getAll" id={"delButton" + i} type="button" className="btn btn-primary" disabled={false} onClick={this.deleteDetail.bind(this, allData.id)}>{this.state.deleteButton}</button></td>
                </tr>
            );
        })
    }
